...
const ProductInfo = ({ product }) => {
  ...
  // Dummy product if none provided
  if (!product) {
    product = {
      ...
      rating: 4.8,
      reviewCount: 42,
      description: 'Our minimalist chair is crafted from sustainable materials with clean lines and timeless design. Perfect for modern spaces, it combines comfort with aesthetic appeal.',
      colors: [
        { name: 'Natural', value: '#E0CCAE' },
  ...
  return (
    <div className="w-full">
      ...
      <div className="flex items-center mb-4">
        {product.discount ? (
          <>
            ...
            <span className="ml-3 text-lg text-gray-400 line-through">${product.discount}</span>
          </>
        ) : (
          <span className="text-2xl font-light text-primary">${product.price}</span>
        )}
      ...
      <div className="flex flex-col sm:flex-row space-y-4 sm:space-y-0 sm:space-x-4 mb-8">
        <div className="flex border border-gray-300">
          <button 
            ...
            className="flex-none w-10 flex items-center justify-center text-gray-600 hover:bg-gray-100"
          >
            <MinusIcon className="w-4 h-4" />
          </button>
          <div className="flex-1 w-12 text-center py-2">{quantity}</div>
          <button 
      ...
      <div className="border-t border-gray-200 pt-6 space-y-2 text-sm text-gray-600">
        <p>Availability: <span className="text-primary">{product.availability}</span></p>
        <p>SKU: {product.sku}</p>
